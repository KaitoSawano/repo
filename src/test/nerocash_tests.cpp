...
BOOST_FIXTURE_TEST_SUITE(nerocash_tests, TestingSetup)

static void TestNerocashBlockSubsidyHalvings(const Consensus::Params& consensusParams)
{
    int maxHalvings = 64;
    ...
    for (int nHalvings = 0; nHalvings < maxHalvings; nHalvings++) {
        ...
        CAmount nSubsidy = GetNerocashBlockSubsidy(nHeight, consensusParams, uint256());
        BOOST_CHECK(nSubsidy <= nInitialSubsidy);
        BOOST_CHECK_EQUAL(nSubsidy, nPreviousSubsidy / 2);
        nPreviousSubsidy = nSubsidy;
    }
    BOOST_CHECK_EQUAL(GetNerocashBlockSubsidy(maxHalvings * consensusParams.nSubsidyHalvingInterval, consensusParams, uint256()), 0);
}

static void TestNerocashBlockSubsidyHalvings(int nSubsidyHalvingInterval)
{
    Consensus::Params consensusParams;
    consensusParams.nSubsidyHalvingInterval = nSubsidyHalvingInterval;
    TestNerocashBlockSubsidyHalvings(consensusParams);
}

BOOST_AUTO_TEST_CASE(block_subsidy_test)
{
    const CChainParams& chainParams = Params(CBaseChainParams::MAIN);
    TestNerocashBlockSubsidyHalvings(chainParams.GetConsensus(0)); // As in main
    TestNerocashBlockSubsidyHalvings(150); // As in regtest
    TestNerocashBlockSubsidyHalvings(1000); // Just another interval
}
...
BOOST_AUTO_TEST_CASE(subsidy_limit_test)
{
    ...
    for (int nHeight = 0; nHeight < 16000000; nHeight += 1000) {
        CAmount nSubsidy = GetNerocashBlockSubsidy(nHeight, chainParams.GetConsensus(nHeight), uint256());
...
BOOST_AUTO_TEST_CASE(subsidy_halving_test)
{
    ...
    BOOST_CHECK_EQUAL(GetNerocashBlockSubsidy(0, consensus, uint256()), 50 * COIN);
    BOOST_CHECK_EQUAL(GetNerocashBlockSubsidy(1280000, consensus, uint256()), 25 * COIN);
    BOOST_CHECK_EQUAL(GetNerocashBlockSubsidy(2560000, consensus, uint256()), (25 * COIN) / 2);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_difficulty_limit)
{
    ...
    BOOST_CHECK_EQUAL(CalculateNerocashNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1e00ffff);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_pre_digishield)
{
    ...
    BOOST_CHECK_EQUAL(CalculateNerocashNextWorkRequired(&pindexLast, nLastRetargetTime, params), 469975616);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_digishield)
{
    ...
    BOOST_CHECK_EQUAL(CalculateNerocashNextWorkRequired(&pindexLast, nLastRetargetTime, params), 454190975);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_digishield_modulated_upper)
{
    ...
    BOOST_CHECK_EQUAL(CalculateNerocashNextWorkRequired(&pindexLast, nLastRetargetTime, params), 453840499);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_digishield_modulated_lower)
{
    ...
    BOOST_CHECK_EQUAL(CalculateNerocashNextWorkRequired(&pindexLast, nLastRetargetTime, params), 454106056);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_digishield_rounding)
{
    ...
    BOOST_CHECK_EQUAL(CalculateNerocashNextWorkRequired(&pindexLast, nLastRetargetTime, params), 454673432);
}
